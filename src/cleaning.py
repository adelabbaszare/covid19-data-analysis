...
def clean_covid_data(df, numeric_columns=None, compute_active=True):
    ...
    if compute_active and {"Confirmed", "Deaths", "Recovered"}.issubset(result.columns):
        ...
        result["Active"] = result["Active"].combine_first(derived_active) if "Active" in result.columns else derived_active
    return result
...
def duplicate_report(df):
    ...
    keys = [c for c in ("Date", "Country_Region", "Province_State", "Admin2") if c in df.columns]
